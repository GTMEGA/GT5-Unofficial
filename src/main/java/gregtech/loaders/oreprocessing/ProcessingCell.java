        ...
        switch (aPrefix) {
                ...
                if (aMaterial == Materials.Empty) {
                    ...
                    if (aMaterial.mFuelPower > 0) {
                        GT_Values.RA.addFuel(
                                ...
                                GT_Utility.getFluidForFilledItem(aStack, true) == null ? GT_Utility.getContainerItem(aStack, true) : null, aMaterial.mFuelPower,
                                aMaterial.mFuelType
                                            );
                    }
                    if ((aMaterial.mMaterialList.size() > 0) && ((aMaterial.mExtraData & 0x3) != 0)) {
                                ...
                                if (tItemAmount + tMat.mAmount * 3628800L <= aStack.getMaxStackSize() * aMaterial.getDensity()) {
                                    ...
                                    if (tStack != null) {
                                        ...
                                        while ((tStack.stackSize > 64) && (
                                                tCapsuleCount + GT_ModHandler.getCapsuleCellContainerCount(tStack) * 64L < 0L ? tList.size() < 5
                                                                                                                              : tList.size() < 6
                                        ...
                                        if ((tStack.stackSize > 0) && tCapsuleCount + GT_ModHandler.getCapsuleCellContainerCountMultipliedWithStackSize(
                                                tStack) <= 64L) {
                                            if (tCapsuleCount + GT_ModHandler.getCapsuleCellContainerCountMultipliedWithStackSize(tStack) < 0L ? tList.size() <
                                                                                                                                                 5
                                                                                                                                               : tList.size() <
                                                                                                                                                 6) {
                        ...
                        if (tList.size() > 0) {
                            if ((aMaterial.mExtraData & 0x1) != 0) {
                                //GT_Values.RA.addElectrolyzerRecipe(GT_Utility.copyAmount(tItemAmount, new Object[] { aStack }), tCapsuleCount >  0L ? (int)
                                // tCapsuleCount : 0, (ItemStack)tList.get(0), tList.size() <  2 ? null : (ItemStack)tList.get(1), tList.size() <  3 ? null :
                                // (ItemStack)tList.get(2), tList.size() <  4 ? null : (ItemStack)tList.get(3), tList.size() <  5 ? null : (ItemStack)tList
                                // .get(4), tList.size()    < 6 ? null : tCapsuleCount < 0L ? ItemList.Cell_Empty.get(-tCapsuleCount, new Object[0]) :
                                // (ItemStack)tList.get(5), (int)Math.max(1L, Math.abs(aMaterial.getProtons() * 8L * tItemAmount)), Math.min(4, tList.size())
                                // * 30);
                                GT_Values.RA.addElectrolyzerRecipe(
                                        GT_Utility.copyAmount(tItemAmount, aStack), tCapsuleCount <= 0L ? 0 : (int) tCapsuleCount, tList.get(0),
                                        tList.size() >= 2 ? tList.get(1) : null, tList.size() >= 3 ? tList.get(2) : null,
                                        tList.size() >= 4 ? tList.get(3) : null, tList.size() >= 5 ? tList.get(4) : null,
                            ...
                            if ((aMaterial.mExtraData & 0x2) != 0) {
                                //GT_Values.RA.addCentrifugeRecipe(GT_Utility.copyAmount(tItemAmount, new Object[] { aStack }), tCapsuleCount > 0L ? (int)
                                // tCapsuleCount : 0, (ItemStack)tList.get(0), tList.size() < 2 ? null : (ItemStack)tList.get(1), tList.size() < 3 ? null :
                                // (ItemStack)tList.get(2), tList.size() < 4 ? null : (ItemStack)tList.get(3), tList.size() < 5 ? null : (ItemStack)tList.get
                                // (4), tList.size() < 6 ? null : tCapsuleCount < 0L ? ItemList.Cell_Empty.get(-tCapsuleCount, new Object[0]) : (ItemStack)
                                // tList.get(5), (int)Math.max(1L, Math.abs(aMaterial.getMass() * 2L * tItemAmount)));
            ...
            case cellPlasma:
                if (aMaterial == Materials.Empty) {
                    ...
                    GT_Values.RA.addFuel(
                            GT_Utility.copyAmount(1L, aStack),
                            GT_Utility.getFluidForFilledItem(aStack, true) == null ? GT_Utility.getContainerItem(aStack, true) : null,
                            (int) Math.max(1024L, 1024L * aMaterial.getMass()), 4
                                        );
                    GT_Values.RA.addVacuumFreezerRecipe(
                            GT_Utility.copyAmount(1L, aStack), gregtech.api.util.GT_OreDictUnificator.get(OrePrefixes.cell, aMaterial, 1L),
