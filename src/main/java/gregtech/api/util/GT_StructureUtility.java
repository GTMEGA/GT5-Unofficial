    ...
    public static <T> IStructureElement<T> ofHatchAdderOptional(
        ...
        return new IStructureElement<T>() {
            ...
            public boolean check(T t, World world, int x, int y, int z) {
                ...
                return (
                               tileEntity instanceof IGregTechTileEntity && aHatchAdder.apply(t, (IGregTechTileEntity) tileEntity, (short) aTextureIndex)
    ...
    public static <T> IStructureElement<T> ofCoil(BiPredicate<T, HeatingCoilLevel> aHeatingCoilSetter, Function<T, HeatingCoilLevel> aHeatingCoilGetter) {
        ...
        return new IStructureElement<T>() {
            ...
            public boolean check(T t, World world, int x, int y, int z) {
                ...
                HeatingCoilLevel existingLevel = aHeatingCoilGetter.apply(t), newLevel = ((IHeatingCoil) block).getCoilHeat(world.getBlockMetadata(x, y, z));
            ...
            private int getMeta(ItemStack trigger) {
                ...
                return GT_Block_Casings5.getMetaFromCoilHeat(
                        HeatingCoilLevel.getFromTier((byte) Math.min(HeatingCoilLevel.getMaxTier() - 4, Math.max(0, trigger.stackSize - 1))));
            }
