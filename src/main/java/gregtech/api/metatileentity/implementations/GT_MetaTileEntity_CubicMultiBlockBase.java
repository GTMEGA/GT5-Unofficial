...
public abstract class GT_MetaTileEntity_CubicMultiBlockBase<T extends GT_MetaTileEntity_CubicMultiBlockBase<T>> extends GT_MetaTileEntity_EnhancedMultiBlockBase<T> {
    ...
    protected static final ClassValue<IStructureDefinition<GT_MetaTileEntity_CubicMultiBlockBase<?>>> STRUCTURE_DEFINITION =
            new ClassValue<IStructureDefinition<GT_MetaTileEntity_CubicMultiBlockBase<?>>>() {

                @Override
                protected IStructureDefinition<GT_MetaTileEntity_CubicMultiBlockBase<?>> computeValue(Class<?> type) {
                    return StructureDefinition.<GT_MetaTileEntity_CubicMultiBlockBase<?>>builder().addShape(STRUCTURE_PIECE_MAIN, transpose(new String[][]{
                            {"hhh", "hhh", "hhh"}, {"h~h", "h-h", "hhh"}, {"hhh", "hhh", "hhh"},
                            })).addElement('h', ofChain(lazy(
                                                                t -> ofHatchAdder(GT_MetaTileEntity_CubicMultiBlockBase::addToMachineList,
                                                                                  t.getHatchTextureIndex(), 1)),
                                                        onElementPass(GT_MetaTileEntity_CubicMultiBlockBase::onCorrectCasingAdded,
    ...
    public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
        mCasingAmount = 0;
        return checkPiece(STRUCTURE_PIECE_MAIN, 1, 1, 0) && mCasingAmount >= getRequiredCasingCount() && checkHatches(aBaseMetaTileEntity, aStack);
    }
