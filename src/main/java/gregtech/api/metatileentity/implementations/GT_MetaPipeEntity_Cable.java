...
public class GT_MetaPipeEntity_Cable extends MetaPipeEntity implements IMetaTileEntityCable {
    ...
    public String[] getDescription() {
        return new String[]{
                "Max Voltage: %%%" + EnumChatFormatting.GREEN + GT_Utility.formatNumbers(mVoltage) + " (" + VN[GT_Utility.getTier(mVoltage)] + ")" +
                EnumChatFormatting.GRAY, "Max Amperage: %%%" + EnumChatFormatting.YELLOW + GT_Utility.formatNumbers(mAmperage) + EnumChatFormatting.GRAY,
    ...
    public ITexture[] getTexture(
        ...
        if (!mInsulated) {
            return new ITexture[]{
                    TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], Dyes.getModulation(aColorIndex, mMaterial.mRGBa))
            };
        }
        if (aConnected) {
            ...
            if (tThickNess < 0.124F) {
                return new ITexture[]{
                        TextureFactory.of(Textures.BlockIcons.INSULATION_FULL, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
            }
            if (tThickNess < 0.374F)//0.375 x1
            {
                return new ITexture[]{
                        TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], mMaterial.mRGBa), TextureFactory.of(
                        Textures.BlockIcons.INSULATION_TINY, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
            }
            if (tThickNess < 0.499F)//0.500 x2
            {
                return new ITexture[]{
                        TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], mMaterial.mRGBa), TextureFactory.of(
                        Textures.BlockIcons.INSULATION_SMALL, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
            }
            if (tThickNess < 0.624F)//0.625 x4
            {
                return new ITexture[]{
                        TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], mMaterial.mRGBa), TextureFactory.of(
                        Textures.BlockIcons.INSULATION_MEDIUM, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
            }
            if (tThickNess < 0.749F)//0.750 x8
            {
                return new ITexture[]{
                        TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], mMaterial.mRGBa), TextureFactory.of(
                        Textures.BlockIcons.INSULATION_MEDIUM_PLUS, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
            }
            if (tThickNess < 0.874F)//0.825 x12
            {
                return new ITexture[]{
                        TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], mMaterial.mRGBa), TextureFactory.of(
                        Textures.BlockIcons.INSULATION_LARGE, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
                };
            }
            return new ITexture[]{
                    TextureFactory.of(mMaterial.mIconSet.mTextures[TextureSet.INDEX_wire], mMaterial.mRGBa), TextureFactory.of(
                    Textures.BlockIcons.INSULATION_HUGE, Dyes.getModulation(aColorIndex, Dyes.CABLE_INSULATION.mRGBa))
            };
        }
    ...
    public String[] getInfoData() {
        ...
        return new String[]{
                ...
                "Heat: " + EnumChatFormatting.RED + GT_Utility.formatNumbers(mOverheat) + EnumChatFormatting.RESET + " / " + EnumChatFormatting.YELLOW +
                GT_Utility.formatNumbers(mMaxOverheat) + EnumChatFormatting.RESET, "Max Load (1t):", EnumChatFormatting.GREEN + GT_Utility.formatNumbers(amps) +
                                                                                                     EnumChatFormatting.RESET + " A / " +
                                                                                                     EnumChatFormatting.YELLOW + GT_Utility.formatNumbers(
                mAmperage) + EnumChatFormatting.RESET + " A", "Max EU/p (1t):", EnumChatFormatting.GREEN + GT_Utility.formatNumbers(volts) +
                                                                                EnumChatFormatting.RESET + " EU / " + EnumChatFormatting.YELLOW +
                                                                                GT_Utility.formatNumbers(mVoltage) + EnumChatFormatting.RESET + " EU",
                "Max Load (20t): " + EnumChatFormatting.GREEN + GT_Utility.formatNumbers(mTransferredAmperageLast20OK) + EnumChatFormatting.RESET + " A",
                "Max EU/p (20t): " + EnumChatFormatting.GREEN + GT_Utility.formatNumbers(mTransferredVoltageLast20OK) + EnumChatFormatting.RESET + " EU"
        };
    }
    ...
    public long injectEnergyUnits(byte aSide, long aVoltage, long aAmperage) {
        ...
        if (!getBaseMetaTileEntity().getCoverBehaviorAtSideNew(aSide).letsEnergyIn(
    ...
    public long transferElectricity(byte aSide, long aVoltage, long aAmperage, HashSet<TileEntity> aAlreadyPassedSet) {
        if (!getBaseMetaTileEntity().isServerSide() || !isConnectedAtSide(aSide) && aSide != 6) {
            return 0;
        }
    ...
    public void onEntityCollidedWithBlock(World aWorld, int aX, int aY, int aZ, Entity aEntity) {
        if (mCanShock && (((BaseMetaPipeEntity) getBaseMetaTileEntity()).mConnections & -128) == 0 && aEntity instanceof EntityLivingBase && !isCoverOnSide(
    ...
    public boolean canConnect(byte aSide, TileEntity tTileEntity) {
        ...
        if (GT_Mod.gregtechproxy.mAE2Integration) {
            if (tTileEntity instanceof appeng.tile.powersink.IC2 && ((appeng.tile.powersink.IC2) tTileEntity).acceptsEnergyFrom(
                    (TileEntity) baseMetaTile, tDir)) {
                return true;
            }
        }
        ...
        if (pipe.getNode() != null) {
            for (byte i = 0; i < 6; i++) {
                if (isConnectedAtSide(i)) {
                    ...
                    if (coverBehavior instanceof GT_Cover_None) {
                        continue;
                    }
