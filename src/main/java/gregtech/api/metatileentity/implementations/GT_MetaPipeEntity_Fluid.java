...
public class GT_MetaPipeEntity_Fluid extends MetaPipeEntity {
    ...
    public ITexture[] getTexture(
        ...
        if (mDisableInput == 0) {
            return new ITexture[]{
                    aConnected ? getBaseTexture(tThickNess, mPipeAmount, mMaterial, aColorIndex) : TextureFactory.of(
        ...
        byte[][] sRestrictionArray = {
                {2, 3, 5, 4}, {2, 3, 4, 5}, {1, 0, 4, 5}, {1, 0, 4, 5}, {1, 0, 2, 3}, {1, 0, 2, 3}
        };
        if (aSide >= 0 && aSide < 6) {
            for (byte i = 0; i < 4; i++) {
        ...
        return new ITexture[]{
                aConnected ? getBaseTexture(tThickNess, mPipeAmount, mMaterial, aColorIndex) : TextureFactory.of(
                        mMaterial.mIconSet.mTextures[OrePrefixes.pipe.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.mRGBa)), getRestrictorTexture(
                tMask)
        };
    }
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (aBaseMetaTileEntity.isServerSide() && aTick % 5 == 0) {
            ...
            for (int i = 0, j = aBaseMetaTileEntity.getRandomNumber(mPipeAmount); i < mPipeAmount; i++) {
                ...
                if (checkEnvironment(index, aBaseMetaTileEntity)) {
                    return;
                }
    ...
    public void doSound(byte aIndex, double aX, double aY, double aZ) {
        ...
        if (aIndex == 9) {
            ...
            new WorldSpawnedEventBuilder.ParticleEventBuilder().setIdentifier("largesmoke").setWorld(getBaseMetaTileEntity().getWorld())
                                                               .<WorldSpawnedEventBuilder.ParticleEventBuilder>times(6, (x, i) -> x.setMotion(
                                                                                                                                           ...
                                                                                                                                           aX - 0.5 + XSTR_INSTANCE.nextFloat(), aY - 0.5 + XSTR_INSTANCE.nextFloat(),
    ...
    private final int fill_default_intoIndex(ForgeDirection aSide, FluidStack aFluid, boolean doFill, int index) {
        ...
        if (!mFluids[index].isFluidEqual(aFluid)) {
            return 0;
        }
        ...
        if (tFluid != null && tFluid.amount > 0) {
            ...
            if (tTemperature > mHeatResistance) {
                if (aBaseMetaTileEntity.getRandomNumber(100) == 0) {
                    // Poof
                    GT_Log.exp.println(
                            "Set Pipe to Fire due to to low heat resistance at " + aBaseMetaTileEntity.getXCoord() + " | " + aBaseMetaTileEntity.getYCoord() +
                            " | " + aBaseMetaTileEntity.getZCoord() + " DIMID: " + aBaseMetaTileEntity.getWorld().provider.dimensionId);
                    aBaseMetaTileEntity.setToFire();
                    return true;
                }
                // Mmhmm, Fire
                aBaseMetaTileEntity.setOnFire();
                GT_Log.exp.println("Set Blocks around Pipe to Fire due to to low heat resistance at " + aBaseMetaTileEntity.getXCoord() + " | " +
                                   aBaseMetaTileEntity.getYCoord() + " | " + aBaseMetaTileEntity.getZCoord() + " DIMID: " +
            ...
            if (!mGasProof && tFluid.getFluid().isGaseous(tFluid)) {
                ...
                if (tTemperature > 320) {
                    try {
                        for (EntityLivingBase tLiving : (ArrayList<EntityLivingBase>) getBaseMetaTileEntity().getWorld().getEntitiesWithinAABB(
                                EntityLivingBase.class,
                                AxisAlignedBB.getBoundingBox(getBaseMetaTileEntity().getXCoord() - 2, getBaseMetaTileEntity().getYCoord() - 2,
                                                             getBaseMetaTileEntity().getZCoord() - 2, getBaseMetaTileEntity().getXCoord() + 3,
                                                             getBaseMetaTileEntity().getYCoord() + 3, getBaseMetaTileEntity().getZCoord() + 3
                                                            )
                                                                                                                                              )) {
    ...
    private void distributeFluid(int index, IGregTechTileEntity aBaseMetaTileEntity) {
        ...
        for (byte aSide, i = 0, j = (byte) aBaseMetaTileEntity.getRandomNumber(6); i < 6; i++) {
            ...
            if (isConnectedAtSide(aSide) && tTank != null && (mLastReceivedFrom & (1 << aSide)) == 0 && getBaseMetaTileEntity().getCoverBehaviorAtSideNew(aSide)
                                                                                                                               .letsFluidOut(
                                                                                                                                       aSide,
                                                                                                                                       getBaseMetaTileEntity().getCoverIDAtSide(
                                                                                                                                               aSide),
                                                                                                                                       getBaseMetaTileEntity().getComplexCoverDataAtSide(
                                                                                                                                               aSide),
                                                                                                                                       tFluid.getFluid(),
                                                                                                                                       getBaseMetaTileEntity()
                                                                                                                                            ) && (
                        gTank == null || gTank.getCoverBehaviorAtSideNew(tSide).letsFluidIn(
                                tSide, gTank.getCoverIDAtSide(tSide), gTank.getComplexCoverDataAtSide(tSide), tFluid.getFluid(), gTank)
                )) {
        ...
        for (MutableTriple<IFluidHandler, ForgeDirection, Integer> tEntry : tTanks) {
            if (availableCapacity > tAmount) {
                tEntry.right = (int) Math.floor(
                        tEntry.right * tAmount / availableCapacity); // Distribue fluids based on percentage available space at destination
            }
            if (tEntry.right == 0) {
            ...
            if (tFilledAmount > 0) {
                tEntry.left.fill(tEntry.middle, drainFromIndex(tFilledAmount, true, index), true);
            }
        }
