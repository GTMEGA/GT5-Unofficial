...
public abstract class GT_MetaTileEntity_BasicMachine extends GT_MetaTileEntity_BasicTank implements IMachineCallback<GT_MetaTileEntity_Cleanroom> {
    ...
    public static boolean isValidForLowGravity(GT_Recipe tRecipe, int dimId) {
        return //TODO check or get a better solution
                DimensionManager.getProvider(dimId).getClass().getName().contains("Orbit") || DimensionManager.getProvider(dimId).getClass().getName().endsWith(
                        "Space") || DimensionManager.getProvider(dimId).getClass().getName().endsWith("Asteroids") || DimensionManager.getProvider(dimId)
                                                                                                                                      .getClass().getName()
                                                                                                                                      .endsWith("SS") ||
    ...
    public boolean mAllowInputFromOutputSide = false, mFluidTransfer = false, mItemTransfer = false, mHasBeenUpdated = false, mStuttering = false, mCharge =
            false, mDecharge = false;
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        return mTextures[mMainFacing < 2 ? aSide == aFacing ? aActive ? 2 : 3 : aSide == 0 ? aActive ? 6 : 7 : aSide == 1 ? aActive ? 4 : 5 : aActive ? 0 : 1
                                         : aSide == mMainFacing ? aActive ? 2 : 3 : (showPipeFacing() && aSide == aFacing) ? aSide == 0 ? aActive ? 8 : 9
                                                                                                                                        : aSide == 1 ? aActive
                                                                                                                                                       ? 10 : 11
                                                                                                                                                     : aActive
                                                                                                                                                       ? 12 : 13
    ...
    public boolean allowPutStack(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, byte aSide, ItemStack aStack) {
        if (aSide == mMainFacing || aIndex < getInputSlot() || aIndex >= getInputSlot() + mInputSlotCount ||
        ...
        for (int i = getInputSlot(), j = i + mInputSlotCount; i < j; i++) {
            if (GT_Utility.areStacksEqual(GT_OreDictUnificator.get(aStack), mInventory[i]) &&
                (mDisableMultiStack || mInventory[i].stackSize + aStack.stackSize <= aStack.getMaxStackSize())) {
                return i == aIndex;
            }
        }
    ...
    public Object getClientGUI(int aID, InventoryPlayer aPlayerInventory, IGregTechTileEntity aBaseMetaTileEntity) {
        return new GT_GUIContainer_BasicMachine(
                aPlayerInventory, aBaseMetaTileEntity, getLocalName(), mGUIName,
        ...
        if (aSide == getBaseMetaTileEntity().getFrontFacing() || aSide == mMainFacing) {
            if (aPlayer.isSneaking()) {
                ...
                GT_Utility.sendChatToPlayer(
                        aPlayer, mAllowInputFromOutputSide ? trans("095", "Input from Output Side allowed") : trans("096", "Input from Output Side forbidden"));
            }
        }
    }
        ...
        if (aBaseMetaTileEntity.isServerSide()) {
            ...
            if (mMaxProgresstime > 0 && (mProgresstime >= 0 || aBaseMetaTileEntity.isAllowedToWork())) {
                ...
                if (mProgresstime < 0 || drainEnergyForProcess(mEUt)) {
                    ...
                    //XSTR aXSTR = new XSTR();
                    //if(GT_Mod.gregtechproxy.mAprilFool && aXSTR.nextInt(5000)==0)GT_Utility.sendSoundToPlayers(aBaseMetaTileEntity.getWorld(), GregTech_API
                    // .sSoundList.get(5), 10.0F, -1.0F, aBaseMetaTileEntity.getXCoord(), aBaseMetaTileEntity.getYCoord(),aBaseMetaTileEntity.getZCoord());
                } else {
                    if (!mStuttering) {
                        stutterProcess();
            ...
            if (doesAutoOutputFluids() && getDrainableStack() != null && aBaseMetaTileEntity.getFrontFacing() != mMainFacing &&
            ...
            if (doesAutoOutput() && !isOutputEmpty() && aBaseMetaTileEntity.getFrontFacing() != mMainFacing &&
                (tSucceeded || mOutputBlocked % 300 == 1 || aBaseMetaTileEntity.hasInventoryBeenModified() || aTick % 600 == 0)) {
                ...
                moveMultipleItemStacks(
                        aBaseMetaTileEntity, tTileEntity2, aBaseMetaTileEntity.getFrontFacing(), aBaseMetaTileEntity.getBackFacing(), null, false, (byte) 64,
                        (byte) 1, (byte) 64, (byte) 1, tMaxStacks
                                      );
//                for (int i = 0, tCosts = 1; i < mOutputItems.length && tCosts > 0 && aBaseMetaTileEntity.isUniversalEnergyStored(128); i++) {
//                    tCosts = GT_Utility.moveOneItemStack(aBaseMetaTileEntity, tTileEntity2, aBaseMetaTileEntity.getFrontFacing(), aBaseMetaTileEntity
//                    .getBackFacing(), null, false, (byte) 64, (byte) 1, (byte) 64, (byte) 1);
...
//                }
            ...
            if (allowToCheckRecipe()) {
                if (mMaxProgresstime <= 0 && aBaseMetaTileEntity.isAllowedToWork() && (
                        tRemovedOutputFluid || tSucceeded || aBaseMetaTileEntity.hasInventoryBeenModified() || aTick % 600 == 0 ||
    ...
    public String[] getInfoData() {
        return new String[]{
                EnumChatFormatting.BLUE + mNEIName + EnumChatFormatting.RESET, "Progress:", EnumChatFormatting.GREEN + GT_Utility.formatNumbers(
                (mProgresstime / 20)) + EnumChatFormatting.RESET + " s / " + EnumChatFormatting.YELLOW + GT_Utility.formatNumbers(mMaxProgresstime / 20) +
                                                                                            EnumChatFormatting.RESET + " s", "Stored Energy:",
                ...
                "Probably uses: " + EnumChatFormatting.RED + GT_Utility.formatNumbers(mEUt) + EnumChatFormatting.RESET + " EU/t at " + EnumChatFormatting.RED +
    ...
    public int checkRecipe(boolean skipOC) {
        ...
        GT_Recipe tRecipe = tMap.findRecipe(
                getBaseMetaTileEntity(), mLastRecipe, false, V[mTier], new FluidStack[]{getFillableStack()}, getSpecialSlot(), getAllInputs());
        if (tRecipe == null) {
            return DID_NOT_FIND_RECIPE;
        }

        if (GT_Mod.gregtechproxy.mLowGravProcessing && (tRecipe.mSpecialValue == -100 || tRecipe.mSpecialValue == -300) && !isValidForLowGravity(tRecipe,
        ...
        if (tRecipe.mSpecialValue == -200 || tRecipe.mSpecialValue == -300) {
            for (int i = 0; i < mOutputItems.length; i++) {
                if (mOutputItems[i] != null && getBaseMetaTileEntity().getRandomNumber(10000) > getCallbackBase().mEfficiency) {
                    if (debugCleanroom) {
                        GT_Log.out.println("BasicMachine: Voiding output due to efficiency failure. mEfficiency = " + getCallbackBase().mEfficiency);
                    }
                    mOutputItems[i] = null;
                }
            }
    ...
    protected boolean canOutput(GT_Recipe aRecipe) {
        return aRecipe != null && (
                aRecipe.mNeedsEmptyOutput ? isOutputEmpty() && getDrainableStack() == null : canOutput(aRecipe.getFluidOutput(0)) && canOutput(aRecipe.mOutputs)
        );
    }
