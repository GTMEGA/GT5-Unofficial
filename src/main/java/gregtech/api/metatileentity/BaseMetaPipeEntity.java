...
public class BaseMetaPipeEntity extends BaseTileEntity implements IGregTechTileEntity, IPipeRenderedTileEntity {
    ...
    public void writeToNBT(NBTTagCompound aNBT) {
        ...
        try {
            ...
            GT_Log.err.println(
                    "Encountered CRITICAL ERROR while saving MetaTileEntity, the Chunk would've been corrupted by now, but I prevented that. Please report " +
                    "immediately to GregTech Intergalactical!!!");
            e.printStackTrace(GT_Log.err);
        }
        try {
    ...
    public void updateEntity() {
        ...
        try {
                            ...
                            if (mTickTimer % 10 == 0) {
                                if (mSendClientData) {
                                    NW.sendPacketToAllPlayersInRange(
                                            worldObj, new GT_Packet_TileEntity(xCoord, (short) yCoord, zCoord, mID, mCoverSides[0], mCoverSides[1],
                                                                               mCoverSides[2], mCoverSides[3], mCoverSides[4], mCoverSides[5],
                                                                               oTextureData = mConnections,
                                                                               oUpdateData = hasValidMetaTileEntity() ? mMetaTileEntity.getUpdateData() : 0,
                                                                               oRedstoneData = (byte) (
                                                                                       ((mSidedRedstone[0] > 0) ? 1 : 0) | ((mSidedRedstone[1] > 0) ? 2 : 0) |
            ...
            //GT_Log.err.println("Encountered Exception while ticking MetaTileEntity in Step " + (tCode - 1) + ". The Game should've crashed now, but I
            // prevented that. Please report immidietly to GregTech Intergalactical!!!");
            e.printStackTrace(GT_Log.err);
        }
    ...
    public ArrayList<String> getDebugInfo(EntityPlayer aPlayer, int aLogLevel) {
        ...
        if (aLogLevel > 1) {
            if (mTimeStatistics.length > 0) {
                ...
                tList.add("Average CPU-load of ~" + (tAverageTime / mTimeStatistics.length) + "ns since " + mTimeStatistics.length +
                          " ticks with worst time of " + tWorstTime + "ns.");
            }
    ...
    public boolean isUseableByPlayer(EntityPlayer aPlayer) {
        return hasValidMetaTileEntity() && mTickTimer > 40 && getTileEntityOffset(0, 0, 0) == this && aPlayer.getDistanceSq(
    ...
    public byte getOutputRedstoneSignal(byte aSide) {
        return (byte) (
                getCoverBehaviorAtSideNew(aSide).manipulatesSidedRedstoneOutput(
                        aSide, getCoverIDAtSide(aSide), getComplexCoverDataAtSide(aSide), this) || (
                        getCoverBehaviorAtSideNew(aSide).letsRedstoneGoOut(
                                aSide, getCoverIDAtSide(aSide), getComplexCoverDataAtSide(aSide), this)
                ) ? mSidedRedstone[aSide] & 15 : 0
        );
    }
    ...
    public boolean canInsertItem(int aIndex, ItemStack aStack, int aSide) {
        return canAccessData() && getCoverBehaviorAtSideNew((byte) aSide).letsItemsIn(
                ...
                if (GT_Utility.isStackInList(tCurrentItem, GregTech_API.sScrewdriverList)) {
                    if (getCoverIDAtSide(aSide) == 0 && getCoverIDAtSide(tSide) != 0) {
                        if (GT_ModHandler.damageOrDechargeItem(tCurrentItem, 1, 200, aPlayer)) {
                            setCoverDataAtSide(
                                    tSide, getCoverBehaviorAtSideNew(tSide).onCoverScrewdriverClick(tSide, getCoverIDAtSide(tSide),
                                                                                                    getComplexCoverDataAtSide(tSide), this, aPlayer, 0.5F, 0.5F,
                                                                                                    0.5F
                                                                                                   ));
                ...
                if (GT_Utility.isStackInList(tCurrentItem, GregTech_API.sSoftHammerList)) {
                    if (GT_ModHandler.damageOrDechargeItem(tCurrentItem, 1, 1000, aPlayer)) {
                        ...
                        GT_Utility.sendChatToPlayer(
                                aPlayer, trans("090", "Machine Processing: ") + (isAllowedToWork() ? trans("088", "Enabled") : trans("087", "Disabled")));
    ...
    private boolean canMoveFluidOnSide(ForgeDirection aSide, Fluid aFluid, boolean isFill) {
        ...
        if (isFill && mMetaTileEntity.isLiquidInput((byte) aSide.ordinal()) && getCoverBehaviorAtSideNew((byte) aSide.ordinal()).letsFluidIn(
                (byte) aSide.ordinal(), getCoverIDAtSide((byte) aSide.ordinal()), getComplexCoverDataAtSide((byte) aSide.ordinal()), aFluid, this)) {
            return true;
        }

        return !isFill && mMetaTileEntity.isLiquidOutput((byte) aSide.ordinal()) && getCoverBehaviorAtSideNew((byte) aSide.ordinal()).letsFluidOut(
                (byte) aSide.ordinal(), getCoverIDAtSide((byte) aSide.ordinal()), getComplexCoverDataAtSide((byte) aSide.ordinal()), aFluid, this);
    }
    ...
    public FluidStack drain(ForgeDirection aSide, int maxDrain, boolean doDrain) {
        if (mTickTimer > 5 && canAccessData() && canMoveFluidOnSide(
                aSide, mMetaTileEntity.getFluid() == null ? null : mMetaTileEntity.getFluid().getFluid(), false)) {
    ...
    public ITexture[] getTextureUncovered(byte aSide) {
        if ((mConnections & IConnectable.HAS_FRESHFOAM) != 0) {
            return Textures.BlockIcons.FRESHFOAM;
        }
        if ((mConnections & IConnectable.HAS_HARDENEDFOAM) != 0) {
        ...
        if (hasValidMetaTileEntity()) {
            return mMetaTileEntity.getTexture(this, aSide, tConnections, (byte) (mColor - 1), tConnections == 0 || (tConnections & (1 << aSide)) != 0,
