...
public abstract class MetaPipeEntity implements IMetaTileEntity, IConnectable {
    ...
    public void doExplosion(long aExplosionPower) {
        ...
        if (GregTech_API.sMachineExplosions) {
            new WorldSpawnedEventBuilder.ExplosionEffectEventBuilder().setStrength(tStrength).setSmoking(true).setPosition(tX + 0.5, tY + 0.5, tZ + 0.5)
                                                                      .setWorld(tWorld).run();
        }
    }
    ...
    public void getWailaNBT(NBTTagCompound tag, World world, BlockCoord pos) {
        ...
        for (byte side = 0; side < 6; side++) {
            if (base.getComplexCoverDataAtSide(side) instanceof GT_Cover_Fluidfilter) {
                tag.setString("filterInfo" + side, ((GT_Cover_Fluidfilter) base.getComplexCoverDataAtSide(side)).getDescription(
                        side, base.getCoverIDAtSide(side), base.getComplexCoverDataAtSide(side), base));
            }
        }
    }
     ...
     */
    public void onCloseGUI() {/*Do nothing*/}    @Override
    ...
    public int connect(byte aSide) {
        ...
        if (!connectableColor(tTileEntity)) {
            return 0;
        }
