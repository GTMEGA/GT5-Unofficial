...
public class GT_SidedTexture implements ITexture, IColorModulationContainer {
    ...
    public GT_SidedTexture(
        ...
        mTextures = new ITexture[]{
                new GT_RenderedTexture(aIcon0, aRGBa, aAllowAlpha), new GT_RenderedTexture(aIcon1, aRGBa, aAllowAlpha), new GT_RenderedTexture(
                aIcon2, aRGBa, aAllowAlpha), new GT_RenderedTexture(aIcon3, aRGBa, aAllowAlpha), new GT_RenderedTexture(aIcon4, aRGBa, aAllowAlpha),
                new GT_RenderedTexture(
                        aIcon5, aRGBa, aAllowAlpha)
        };
        mRGBa = aRGBa;
    }
    ...
    public boolean isValidTexture() {
        for (ITexture renderedTexture : mTextures) {
            if (!renderedTexture.isValidTexture()) {
                return false;
            }
        }
        return true;
