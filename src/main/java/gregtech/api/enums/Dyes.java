...
public enum Dyes implements IColorModulationContainer {
    ...
    public static final Dyes[] VALUES = {
            dyeBlack, dyeRed, dyeGreen, dyeBrown, dyeBlue, dyePurple, dyeCyan, dyeLightGray, dyeGray, dyePink, dyeLime, dyeYellow, dyeLightBlue, dyeMagenta,
            dyeOrange, dyeWhite
    };

    public static Dyes get(int aColor) {
        if (aColor >= 0 && aColor < 16) {
            return VALUES[aColor];
        }
        return _NULL;
    }
    ...
    public static boolean isAnyFluidDye(Fluid aFluid) {
        if (aFluid != null) {
            for (Dyes tDye : VALUES) {
                if (tDye.isFluidDye(aFluid)) {
                    return true;
                }
            }
        }
    ...
    public boolean addFluidDye(Fluid aDye) {
        if (aDye == null || mFluidDyes.contains(aDye)) {
            return false;
        }
        mFluidDyes.add(aDye);
        return true;
    ...
    public FluidStack getFluidDye(int aIndex, long aAmount) {
        if (aIndex >= mFluidDyes.size() || aIndex < 0) {
            return null;
        }
