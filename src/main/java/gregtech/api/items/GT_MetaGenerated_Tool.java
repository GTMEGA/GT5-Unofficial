...
public abstract class GT_MetaGenerated_Tool extends GT_MetaBase_Item implements IDamagableItem, IToolGrafter, IToolCrowbar, IToolWrench, ITool {
    ...
    public void onHarvestBlockEvent(
        ...
        if (isItemStackUsable(aStack) && getDigSpeed(aStack, aBlock, aMetaData) > 0.0F) {
            doDamage(aStack, (long) tStats.convertBlockDrops(aDrops, aStack, aPlayer, aBlock, aX, aY, aZ, aMetaData, aFortune, aSilkTouch, aEvent) *
    ...
    public boolean onLeftClickEntity(ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        ...
        if (aEntity.canAttackWithItem() && !aEntity.hitByEntity(aPlayer)) {
            float tMagicDamage = tStats.getMagicDamageAgainstEntity(
                    aEntity instanceof EntityLivingBase ? EnchantmentHelper.getEnchantmentModifierLiving(aPlayer, (EntityLivingBase) aEntity) : 0.0F, aEntity,
                    aStack, aPlayer
                                                                   ), tDamage = tStats.getNormalDamageAgainstEntity((float) aPlayer.getEntityAttribute(
                    SharedMonsterAttributes.attackDamage).getAttributeValue() + getToolCombatDamage(aStack), aEntity, aStack, aPlayer);
            if (tDamage + tMagicDamage > 0.0F) {
                boolean tCriticalHit =
                        aPlayer.fallDistance > 0.0F && !aPlayer.onGround && !aPlayer.isOnLadder() && !aPlayer.isInWater() && !aPlayer.isPotionActive(
                                Potion.blindness) && aPlayer.ridingEntity == null && aEntity instanceof EntityLivingBase;
                ...
                if (aEntity.attackEntityFrom(tStats.getDamageSource(aPlayer, aEntity), tDamage)) {
                    ...
                    int tKnockcack = (aPlayer.isSprinting() ? 1 : 0) + (
                            aEntity instanceof EntityLivingBase ? EnchantmentHelper.getKnockbackModifier(
                                    aPlayer, (EntityLivingBase) aEntity) : 0
                    );
                    if (tKnockcack > 0) {
                        aEntity.addVelocity(
                                -MathHelper.sin(aPlayer.rotationYaw * (float) Math.PI / 180.0F) * tKnockcack * 0.5F, 0.1D,
    ...
    public Long[] getElectricStats(ItemStack aStack) {
        ...
        if (aNBT != null) {
            ...
            if (aNBT != null && aNBT.getBoolean("Electric")) {
                return new Long[]{
                        aNBT.getLong("MaxCharge"), aNBT.getLong("Voltage"), aNBT.getLong("Tier"), aNBT.getLong("SpecialData")
                };
            }
        }
        return null;
...
//    }
    ...
    public void addAdditionalToolTips(List aList, ItemStack aStack, EntityPlayer aPlayer) {
        ...
        if (tStats != null) {
            if (tStats instanceof GT_Tool_Turbine) {
                int aOptFlow = GT_Utility.safeInt((long) Math.max(Float.MIN_NORMAL,
                                                                  ((GT_MetaGenerated_Tool) aStack.getItem()).getToolStats(aStack).getSpeedMultiplier() *
                                                                  ((GT_MetaGenerated_Tool) aStack.getItem()).getPrimaryMaterial(aStack).mToolSpeed * 50
                                                                 ));
                aList.add(tOffset, EnumChatFormatting.WHITE + String.format(trans("001", "Durability: %s/%s"),
                                                                              "" + EnumChatFormatting.GREEN + (tMaxDamage - getToolDamage(aStack)) + " ",
                                                                              " " + tMaxDamage
                                                                           ) + EnumChatFormatting.GRAY);
                aList.add(
                        tOffset + 1, EnumChatFormatting.WHITE + String.format(trans("002", "%s lvl %s"), tMaterial.mLocalizedName + EnumChatFormatting.YELLOW,
                                                                              "" + getHarvestLevel(aStack, "")
                                                                             ) + EnumChatFormatting.GRAY);
                aList.add(
                        tOffset + 2, EnumChatFormatting.WHITE + String.format(trans("005", "Turbine Efficiency: %s"),
                                                                              "" + EnumChatFormatting.BLUE + (50.0F + (10.0F * getToolCombatDamage(aStack)))
                                                                             ) + EnumChatFormatting.GRAY);
                aList.add(
                        tOffset + 3, EnumChatFormatting.WHITE + String.format(trans("006", "Optimal Steam flow: %sL/sec"), "" + EnumChatFormatting.GOLD +
                                                                                                                           ...
                                                                                                                           EnumChatFormatting.GRAY));
                {
                    ...
                    aList.add(
                            tOffset + 4, EnumChatFormatting.GRAY +
                                         String.format(trans("500", "Turbine Efficiency (Loose): %s"), "" + EnumChatFormatting.BLUE + aBaseEff / 100f) +
                                         EnumChatFormatting.DARK_GRAY);
                    aList.add(
                            tOffset + 5, EnumChatFormatting.GRAY + String.format(trans("501", "Optimal Steam flow (Loose): %s L/t"),
                                                                                 "" + EnumChatFormatting.GOLD + aOptFlowLoose + EnumChatFormatting.DARK_GRAY
                                                                                ));
                }
                aList.add(
                        tOffset + 6, EnumChatFormatting.WHITE + String.format(trans("007", "Optimal Gas flow(EU burnvalue per tick): %sEU/t"),
                                                                              "" + EnumChatFormatting.GOLD + Math.max(Float.MIN_NORMAL,
                                                                                                                      tStats.getSpeedMultiplier() *
                                                                                                                      getPrimaryMaterial(aStack).mToolSpeed * 50
                                                                                                                     ) + EnumChatFormatting.GRAY
                                                                             ));
                aList.add(
                        tOffset + 7, EnumChatFormatting.WHITE + String.format(trans("008", "Optimal Plasma flow(Plasma energyvalue per tick): %sEU/t"),
                                                                              "" + EnumChatFormatting.GOLD + Math.max(Float.MIN_NORMAL,
                                                                                                                      tStats.getSpeedMultiplier() *
                ...
                aList.add(
                        tOffset + 2, EnumChatFormatting.WHITE +
                                     String.format(trans("003", "Attack Damage: %s"), "" + EnumChatFormatting.BLUE + getToolCombatDamage(aStack)) +
                                     EnumChatFormatting.GRAY);
                aList.add(
                        tOffset + 3, EnumChatFormatting.WHITE + String.format(trans("004", "Mining Speed: %s"), "" + EnumChatFormatting.GOLD +
                                                                                                                Math.max(Float.MIN_NORMAL,
                                                                                                                         tStats.getSpeedMultiplier() *
                                                                                                                         getPrimaryMaterial(aStack).mToolSpeed
                                                                                                                        )) + EnumChatFormatting.GRAY);
    ...
    public final boolean doDamage(ItemStack aStack, long aAmount) {
        ...
        return false;/* use(aStack, (int) aAmount, null); */
    }
    ...
    public float getDigSpeed(ItemStack aStack, Block aBlock, int aMetaData) {
        ...
        float digSpeed = tStats.isMinableBlock(aBlock, (byte) aMetaData) ? Math.max(
                Float.MIN_NORMAL, tStats.getSpeedMultiplier() * getPrimaryMaterial(aStack).mToolSpeed) : 0.0F;
        if (tStats instanceof IAOETool) {
        ...
        if (tStats.isChainsaw() && (aBlock instanceof IShearable)) {
            ...
            if ((target.isShearable(aStack, aPlayer.worldObj, aX, aY, aZ))) {
                ArrayList<ItemStack> drops = target.onSheared(
                        aStack, aPlayer.worldObj, aX, aY, aZ, EnchantmentHelper.getEnchantmentLevel(Enchantment.fortune.effectId, aStack));
    ...
    public boolean canWrench(EntityPlayer player, int x, int y, int z) {
        ...
        if (!isItemStackUsable(player.getCurrentEquippedItem())) {
            return false;
        }
    ...
    public void wrenchUsed(EntityPlayer player, int x, int y, int z) {
        ...
        IToolStats tStats = getToolStats(player.getCurrentEquippedItem());
        if (tStats != null) {
            doDamage(player.getCurrentEquippedItem(), tStats.getToolDamagePerEntityAttack());
        }
    }
