    ...
    public boolean executeWorldgen(
        ...
        for (int x = aChunkX / 16 - windowWidth; x < (aChunkX / 16 + windowWidth + 1); x++) {
            for (int z = aChunkZ / 16 - windowWidth; z < (aChunkZ / 16 + windowWidth + 1); z++) {
                long hash =
                        (((aWorld.provider.dimensionId & 0xffL) << 56) | (((long) x & 0x000000000fffffffL) << 28) | ((long) z & 0x000000000fffffffL));
                if (!validStoneSeeds.containsKey(hash)) {
                    // Determine if RNG says to add stone at this chunk
                    stoneRNG.setSeed(aWorld.getSeed() ^ hash + Math.abs(mBlockMeta) + Math.abs(mSize) + (
                            (GregTech_API.sBlockGranites == mBlock) ? (32768) : (0)
                    ));  //Don't judge me. Want different values for different block types
                    if ((this.mProbability <= 1) || (stoneRNG.nextInt(this.mProbability) == 0)) {
                        ...
                        if (debugStones) {
                            GT_Log.out.println("New stoneseed=" + mWorldGenName + " x=" + x + " z=" + z + " realSize=" + realSize);
                        }
                    } else {
        ...
        for (; stones.size() != 0; stones.remove(0)) {
            ...
            stoneRNG.setSeed(aWorld.getSeed() ^ (
            ...
            for (int i = 0; i < this.mAmount; i++) { // Not sure why you would want more than one in a chunk! Left alone though.
                ...
                if (aWorld.getBlock(aChunkX + 8, tMinY, aChunkZ + 8).isAir(aWorld, aChunkX + 8, tMinY, aChunkZ + 8)) {
                    if (debugStones) {
                        GT_Log.out.println(
                                mWorldGenName + " tX=" + tX + " tY=" + tY + " tZ=" + tZ + " realSize=" + realSize + " xSize=" + realSize / xSize + " ySize=" +
                                realSize / ySize + " zSize=" + realSize / zSize + " tMinY=" + tMinY + " tMaxY=" + tMaxY +
                ...
                if (debugStones) {
                    GT_Log.out.println(
                            mWorldGenName + " tX=" + tX + " tY=" + tY + " tZ=" + tZ + " realSize=" + realSize + " xSize=" + realSize / xSize + " ySize=" +
                            realSize / ySize + " zSize=" + realSize / zSize + " wX=" + wX + " eX=" + eX + " tMinY=" + tMinY + " tMaxY=" + tMaxY + " sZ=" + sZ +
                            " nZ=" + nZ);
                }
                        ...
                        for (int iZ = sZ; iZ < nZ; iZ++) {
                            ...
                            if (leftHandSize > rightHandSide) {
                                ...
                                if (((this.mAllowToGenerateinVoid) && (aWorld.getBlock(iX, iY, iZ).isAir(aWorld, iX, iY, iZ))) || (
                                        (tTargetedBlock != null) && (
                                                GT_Utility.isBlockReplaceableForOreGeneration(
                                                        aWorld, iX, iY, iZ, tTargetedBlock, Blocks.stone, Blocks.stained_hardened_clay, Blocks.cobblestone,
                                                        Blocks.end_stone, Blocks.netherrack, GregTech_API.sBlockGranites, GregTech_API.sBlockStones
                                                                                             )
                                        )
                                )) {
