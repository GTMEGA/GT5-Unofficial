...
public class GT_MetaTileEntity_Charcoal_Pit extends GT_MetaTileEntity_MultiBlockBase {
    ...
    private boolean checkRecursiveBlocks() {
        ...
        if (running) {
            for (ChunkPosition tPos : tList1) {
                if (isWoodLog(getBaseMetaTileEntity().getBlockOffset(tPos.chunkPosX, tPos.chunkPosY, tPos.chunkPosZ),
                              getBaseMetaTileEntity().getMetaIDOffset(tPos.chunkPosX, tPos.chunkPosY, tPos.chunkPosZ)
                             )) {
                    getBaseMetaTileEntity().getWorld().setBlock(getBaseMetaTileEntity().getXCoord() + tPos.chunkPosX,
                                                                getBaseMetaTileEntity().getYCoord() + tPos.chunkPosY,
                                                                getBaseMetaTileEntity().getZCoord() + tPos.chunkPosZ, GregTech_API.sBlockReinforced, 4, 3
                                                               );
                }
            }
            running = false;
    ...
    public String[] getDescription() {
        ...
        tt.addMachineType("Charcoal Pile Igniter").addInfo("Controller for the Charcoal Pit").addInfo("Converts Logs into Brittle Charcoal blocks").addInfo(
                  "Will automatically start when valid").addPollutionAmount(100).addSeparator().beginVariableStructureBlock(3, 11, 3, 6, 3, 11, false)
          .addStructureInfo("Can be up to 11x6x11 in size, shape doesn't matter").addOtherStructurePart("Bricks", "Bottom layer, under all wood logs")
          .addOtherStructurePart("Dirt/Grass", "All logs must be covered by these, the controller, or bricks").addOtherStructurePart(
                  "Wood Logs", "Inside the previously mentioned blocks").addStructureInfo("No air between logs allowed").toolTipFinisher("Gregtech");
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        if (aSide == 1) {
            if (aActive) {
                return new ITexture[]{
                        casingTexturePages[0][10], TextureFactory.of(OVERLAY_FRONT_ROCK_BREAKER_ACTIVE), TextureFactory.builder().addIcon(
                        OVERLAY_FRONT_ROCK_BREAKER_ACTIVE_GLOW).glow().build()
                };
            }
            return new ITexture[]{
                    casingTexturePages[0][10], TextureFactory.of(OVERLAY_FRONT_ROCK_BREAKER), TextureFactory.builder().addIcon(OVERLAY_FRONT_ROCK_BREAKER_GLOW)
                                                                                                            .glow().build(),
                    };
        }
