...
public abstract class GT_MetaTileEntity_DrillerBase extends GT_MetaTileEntity_EnhancedMultiBlockBase<GT_MetaTileEntity_DrillerBase> implements IChunkLoader {
    ...
    protected static final ClassValue<IStructureDefinition<GT_MetaTileEntity_DrillerBase>> STRUCTURE_DEFINITION =
            new ClassValue<IStructureDefinition<GT_MetaTileEntity_DrillerBase>>() {
                ...
                protected IStructureDefinition<GT_MetaTileEntity_DrillerBase> computeValue(Class<?> type) {
                    return StructureDefinition.<GT_MetaTileEntity_DrillerBase>builder().addShape(STRUCTURE_PIECE_MAIN, transpose(new String[][]{
                            {"   ", " f ", "   "}, {"   ", " f ", "   "}, {"   ", " f ", "   "}, {" f ", "fcf", " f "}, {" f ", "fcf", " f "},
                            {" f ", "fcf", " f "}, {"b~b", "bbb", "bbb"},
                            })).addElement('f', lazy(t -> ofFrame(t.getFrameMaterial()))).addElement(
                            'c', lazy(t -> ofBlock(t.getCasingBlockItem().getBlock(), t.getCasingBlockItem().get(0).getItemDamage()))).addElement(
                            'b', lazy(t -> ofChain(ofBlock(
                                    t.getCasingBlockItem().getBlock(), t.getCasingBlockItem().get(0).getItemDamage()), ofHatchAdder(
                                    GT_MetaTileEntity_DrillerBase::addMaintenanceToMachineList, t.casingTextureIndex, 1), ofHatchAdder(
                                    GT_MetaTileEntity_DrillerBase::addInputToMachineList, t.casingTextureIndex, 1), ofHatchAdder(
                                    GT_MetaTileEntity_DrillerBase::addOutputToMachineList, t.casingTextureIndex, 1), ofHatchAdder(
                                    GT_MetaTileEntity_DrillerBase::addEnergyInputToMachineList, t.casingTextureIndex, 1), ofHatchAdder(
                                    GT_MetaTileEntity_DrillerBase::addDataAccessToMachineList, t.casingTextureIndex, 1)))).build();
                }
            };
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) {
            if (aActive) {
                return new ITexture[]{
                        getCasingTextureForId(casingTextureIndex), TextureFactory.builder().addIcon(OVERLAY_FRONT_ORE_DRILL_ACTIVE).extFacing().build(),
                        TextureFactory.builder().addIcon(OVERLAY_FRONT_ORE_DRILL_ACTIVE_GLOW).extFacing().glow().build()
                };
            }
            return new ITexture[]{
                    getCasingTextureForId(casingTextureIndex), TextureFactory.builder().addIcon(OVERLAY_FRONT_ORE_DRILL).extFacing().build(),
    ...
    public boolean onSolderingToolRightClick(byte aSide, byte aWrenchingSide, EntityPlayer aPlayer, float aX, float aY, float aZ) {
        if (aSide == getBaseMetaTileEntity().getFrontFacing()) {
            ...
            GT_Utility.sendChatToPlayer(
                    aPlayer, mChunkLoadingEnabled ? trans("502", "Mining chunk loading enabled") : trans("503", "Mining chunk loading disabled"));
            return true;
        }
    ...
    protected int tryLowerPipeState(boolean isSimulating) {
        ...
        if (b != miningPipeTipBlock && !GT_Utility.setBlockByFakePlayer(
                getFakePlayer(getBaseMetaTileEntity()), xPipe, yHead - 1, zPipe, miningPipeTipBlock, 0, isSimulating)) {
            return 3;
        }
        if (!isSimulating) {
            if (yHead != yDrill) {
    ...
    private boolean checkBlockAndMeta(int x, int y, int z, Block block, int meta) {
        return (meta == W || getBaseMetaTileEntity().getMetaID(x, y, z) == meta) && getBaseMetaTileEntity().getBlock(x, y, z) == block;
    }
    ...
    private boolean tryOutputPipe() {
        if (!getBaseMetaTileEntity().addStackToSlot(1, GT_Utility.copyAmount(1, miningPipe))) {
            mOutputItems = new ItemStack[]{
                    GT_Utility.copyAmount(1, miningPipe)
            };
        }
        return true;
    }
