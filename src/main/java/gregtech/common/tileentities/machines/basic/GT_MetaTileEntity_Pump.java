...
public class GT_MetaTileEntity_Pump extends GT_MetaTileEntity_Hatch {
    ...
    public GT_MetaTileEntity_Pump(int aID, String aName, String aNameRegional, int aTier) {
        super(aID, aName, aNameRegional, aTier, 3, new String[]{
                ...
                ), "Use Screwdriver to regulate pumping area", "Disable itself upon hitting rocks", "Disable the bottom pump to retract the pipe!"
        });
    ...
    public void onScrewdriverRightClick(byte aSide, EntityPlayer aPlayer, float aX, float aY, float aZ) {
        ...
        GT_Utility.sendChatToPlayer(
                aPlayer,
                StatCollector.translateToLocal("GT5U.machines.workareaset") + " " + (radiusConfig * 2 + 1) + "x" + (radiusConfig * 2 + 1)
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            for (
                    int i = 1; (i < 21) && ((tTileEntity = getBaseMetaTileEntity().getIGregTechTileEntityAtSideAndDistance((byte) 0, i)) != null) &&
                    ...
                    if ((getBaseMetaTileEntity().isUniversalEnergyStored(this.getEuUsagePerAction())) &&
                        ...
                        if ((this.mPumpList.isEmpty()) && (getBaseMetaTileEntity().getTimer() % 100L == 0L)) {
                            if (!this.wasPumping) {
                                ...
                                if (!tMovedOneDown) {
                                    if (canMoveDown(
                                            getBaseMetaTileEntity().getXCoord(), Math.max(getYOfPumpHead() - 1, 1), getBaseMetaTileEntity().getZCoord())) {
                                        if (debugBlockPump) {
                        ...
                        if (!this.hasValidFluid()) {
                            ...
                            if (debugBlockPump && this.mPrimaryPumpedBlock != null) {
                                GT_Log.out.println("PUMP: Had an invalid pump block. Trying to find a fluid at Y: " + y + " Previous blocks 1: " +
                                                   this.mPrimaryPumpedBlock + " 2: " + this.mSecondaryPumpedBlock);
                            }
                            // First look down
                            ...
                            if ((tMovedOneDown) || this.wasPumping || ((this.mPumpList.isEmpty()) && (getBaseMetaTileEntity().getTimer() % 200L == 100L)) ||
                                ...
                                if (debugBlockPump) {
                                    GT_Log.out.println(
                                            "PUMP: Rebuilding pump list - Size " + this.mPumpList.size() + " WasPumping: " + this.wasPumping + " Timer " +
    ...
    public ArrayList<String> getSpecialDebugInfo(IGregTechTileEntity aBaseMetaTileEntity, EntityPlayer aPlayer, int aLogLevel, ArrayList<String> aList) {
        aList.addAll(Arrays.asList(EnumChatFormatting.BLUE + StatCollector.translateToLocal("GT5U.machines.pump") + EnumChatFormatting.RESET,
                                   ...
                                   "Pump timer: " + mPumpTimer, "Meta Entity Timer: " + getBaseMetaTileEntity().getTimer()
                                  ));
        return aList;
    ...
    public String[] getInfoData() {
        return new String[]{
                EnumChatFormatting.BLUE + StatCollector.translateToLocal("GT5U.machines.pump") + EnumChatFormatting.RESET, StatCollector.translateToLocal(
                "GT5U.machines.workarea") + ": " + EnumChatFormatting.GREEN + (radiusConfig * 2 + 1) + EnumChatFormatting.RESET + " " +
                                                                                                                           StatCollector.translateToLocal(
                                                                                                                                   "GT5U.machines.blocks")
        };
    }
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        return new ITexture[]{
                Textures.BlockIcons.MACHINE_CASINGS[mTier][aColorIndex + 1], (aSide == 0 || aSide == 1) ? TextureFactory.of(
                Textures.BlockIcons.OVERLAY_PIPE_OUT) : TextureFactory.of(Textures.BlockIcons.OVERLAY_ADV_PUMP)
        };
    }
    ...
    public ITexture[] getTexturesInactive(ITexture aBaseTexture) {
        return new ITexture[]{
                TextureFactory.of(Textures.BlockIcons.OVERLAY_ADV_PUMP), TextureFactory.of(Textures.BlockIcons.OVERLAY_ADV_PUMP), TextureFactory.of(
                Textures.BlockIcons.OVERLAY_ADV_PUMP), TextureFactory.of(Textures.BlockIcons.OVERLAY_ADV_PUMP),
                };
    }
    ...
    public void saveNBTData(NBTTagCompound aNBT) {
        ...
        if (debugBlockPump) {
            GT_Log.out.println(
                    "PUMP: NBT:Save - WasPumping - " + wasPumping + " blocks (" + this.mPrimaryPumpedBlock + ", " + this.mSecondaryPumpedBlock + ")");
        }
    ...
    private boolean moveOneDown() {
        ...
        if ((!consumeFluid(x, yHead - 1, z)) && (
                !getBaseMetaTileEntity().getBlock(x, yHead - 1, z).isAir(
                        getBaseMetaTileEntity().getWorld(), x, yHead - 1, z)
        )) {
    ...
    private boolean canMoveDown(int aX, int aY, int aZ) {
        if (!GT_Utility.eraseBlockByFakePlayer(getFakePlayer(getBaseMetaTileEntity()), aX, aY, aZ, true)) {
            return false;
        }
        ...
        return aBlock != null && (
                aBlock == Blocks.water || aBlock == Blocks.flowing_water || aBlock == Blocks.lava || aBlock == Blocks.flowing_lava ||
                aBlock instanceof IFluidBlock || aBlock.isAir(getBaseMetaTileEntity().getWorld(), aX, aY, aZ)
        );
    }
    ...
    private boolean consumeFluid(int aX, int aY, int aZ) {
        ...
        if (aBlock != null && ((this.mPrimaryPumpedBlock == aBlock) || (this.mSecondaryPumpedBlock == aBlock))) {
            ...
            } else if (GT_ModHandler.isWater(this.mFluid) || GT_ModHandler.isLava(this.mFluid) || this.mFluid.isFluidEqual(
