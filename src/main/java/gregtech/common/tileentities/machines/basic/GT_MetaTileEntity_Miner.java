...
public class GT_MetaTileEntity_Miner extends GT_MetaTileEntity_BasicMachine {
    ...
    public GT_MetaTileEntity_Miner(int aID, String aName, String aNameRegional, int aTier) {
        super(aID, aName, aNameRegional, aTier, 1, new String[]{
                      "Digging ore instead of you", "Use Screwdriver to regulate work area",
                      ENERGY[aTier] + " EU/t, " + SPEED[aTier] / 20 + " sec per block, no stuttering",
                      "Maximum work area " + (RADIUS[aTier] * 2 + 1) + "x" + (RADIUS[aTier] * 2 + 1), "Fortune bonus of " + aTier
              }, 2, 2, "Miner.png", "", TextureFactory.of(new Textures.BlockIcons.CustomIcon("basicmachines/miner/OVERLAY_SIDE_ACTIVE")),
    ...
    public boolean moveOneDown(IGregTechTileEntity aBaseMetaTileEntity) {
        ...
        if (yCoord + drillY - 1 < 0 || GT_Utility.getBlockHardnessAt(aBaseMetaTileEntity.getWorld(), xCoord, yCoord + drillY - 1, zCoord) < 0 ||
...
miningPipes:
    ...
    public void onScrewdriverRightClick(byte aSide, EntityPlayer aPlayer, float aX, float aY, float aZ) {
        ...
        if (aSide != getBaseMetaTileEntity().getFrontFacing() && aSide != mMainFacing) {
            ...
            GT_Utility.sendChatToPlayer(
                    aPlayer, StatCollector.translateToLocal("GT5U.machines.workareaset") + " " + (radiusConfig * 2 + 1) + "x" +
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (aBaseMetaTileEntity.isServerSide()) {
            if (aBaseMetaTileEntity.isAllowedToWork() && aBaseMetaTileEntity.isUniversalEnergyStored((long) ENERGY[mTier] * (SPEED[mTier] - mProgresstime)) &&
                hasFreeSpace()) {
miningPipe:
                if (waitMiningPipe) {
                    mMaxProgresstime = 0;
                ...
                if (debugBlockMiner) {
                    ...
                    if (!aBaseMetaTileEntity.isUniversalEnergyStored((long) ENERGY[mTier] * (SPEED[mTier] - mProgresstime))) {
                        GT_Log.out.println("MINER: Not enough energy yet, want " + (ENERGY[mTier] * SPEED[mTier]) + " have " +
            ...
            if (mProgresstime == SPEED[mTier] - 1) {
                if (isPickingPipes) {
                    ...
                    } else if (aBaseMetaTileEntity.getBlockOffset(0, drillY, 0) == MINING_PIPE_TIP_BLOCK || aBaseMetaTileEntity.getBlockOffset(0, drillY, 0) ==
                                                                                                            MINING_PIPE_BLOCK) {
                        mOutputItems[0] = MINING_PIPE.copy();
                        mOutputItems[0].stackSize = 1;
                        aBaseMetaTileEntity.getWorld().setBlockToAir(
                                aBaseMetaTileEntity.getXCoord(), aBaseMetaTileEntity.getYCoord() + drillY, aBaseMetaTileEntity.getZCoord());
                        drillY++;
                    }
                    return;
                }
    ...
    public String[] getInfoData() {
        return new String[]{
                EnumChatFormatting.BLUE + StatCollector.translateToLocal("GT5U.machines.miner") + EnumChatFormatting.RESET, StatCollector.translateToLocal(
