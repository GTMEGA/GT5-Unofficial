...
public class GT_MetaTileEntity_Disassembler extends GT_MetaTileEntity_BasicMachine {
    ...
    private static final Object[][] OreDictionaryOverride = {
            {
                    ...
                    //ItemStack to replace
                    new ItemStack(Blocks.planks), new ItemStack(Blocks.cobblestone), new ItemStack(Items.diamond), new ItemStack(Blocks.log), new ItemStack(
            Items.stick), new ItemStack(Blocks.sapling)
            }
    };
    ...
    private static void handleInputStacks(Set<ItemStack[]> inputsStacks, AtomicReference<? super Materials> toRpl, ItemData data, Materials first, int i) {
        ...
        inputsStacks.forEach(stackArray -> {
            ...
            if (dataAgainst == null || dataAgainst.mMaterial == null || dataAgainst.mMaterial.mMaterial == null || dataAgainst.mPrefix == null ||
    ...
    private static double getCheaperInputs(GT_MetaTileEntity_Disassembler.DissassembleReference x) {
        ...
        double inputValue = Arrays.stream(x.inputs).flatMapToInt(f -> IntStream.of(f.stackSize)).sum() + (fluidInputValueRaw / 144D);
        ...
        double outputValue = Arrays.stream(x.recipe.mOutputs).flatMapToInt(f -> IntStream.of(f.stackSize)).sum() + (fluidOutputValueRaw / 144D);
    ...
    public GT_MetaTileEntity_Disassembler(int aID, String aName, String aNameRegional, int aTier) {
        super(aID, aName, aNameRegional, aTier, 1, new String[]{
                      "Disassembles Machines up to " + GT_Values.TIER_COLORS[aTier] + GT_Values.VOLTAGE_NAMES[aTier], "Can also disassemble most assembler " +
                                                                                                                      "recipes!"
              }, 1, 9, "Disassembler.png", "",

              //Textures
              TextureFactory.of(TextureFactory.of(OVERLAY_SIDE_DISASSEMBLER_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_SIDE_DISASSEMBLER_ACTIVE_GLOW)
                                                                                                   .glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_SIDE_DISASSEMBLER), TextureFactory.builder().addIcon(OVERLAY_SIDE_DISASSEMBLER_GLOW).glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_FRONT_DISASSEMBLER_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_FRONT_DISASSEMBLER_ACTIVE_GLOW)
                                                                                                    .glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_FRONT_DISASSEMBLER), TextureFactory.builder().addIcon(OVERLAY_FRONT_DISASSEMBLER_GLOW).glow()
                                                                                             .build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_TOP_DISASSEMBLER_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_TOP_DISASSEMBLER_ACTIVE_GLOW)
                                                                                                  .glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_TOP_DISASSEMBLER), TextureFactory.builder().addIcon(OVERLAY_TOP_DISASSEMBLER_GLOW).glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_BOTTOM_DISASSEMBLER_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_BOTTOM_DISASSEMBLER_ACTIVE_GLOW)
                                                                                                     .glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_BOTTOM_DISASSEMBLER), TextureFactory.builder().addIcon(OVERLAY_BOTTOM_DISASSEMBLER_GLOW).glow()
                                                                                              .build())
             );
    }
    ...
    protected boolean allowPutStackValidated(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, byte aSide, ItemStack aStack) {
        return super.allowPutStackValidated(aBaseMetaTileEntity, aIndex, aSide, aStack) && aStack.getTagCompound() != null &&
    ...
    public int checkRecipe() {
        ...
        if (is.getItem() instanceof GT_MetaGenerated_Tool || blackList.contains(new GT_ItemStack(is)) || compareToUnpacker(is)) {
            return DID_NOT_FIND_RECIPE;
        }
    ...
    private Integer handleHardOverride(ItemStack is) {
        ...
        for (GT_ItemStack stack : stacks) {
            ...
            if (stack.isStackEqual(in) && stack.mStackSize <= is.stackSize) {
                return setOutputsAndTime(outputHardOverrides.get(stack).toArray(new ItemStack[0]), stack.mStackSize) ? FOUND_AND_SUCCESSFULLY_USED_RECIPE
                                                                                                                     : DID_NOT_FIND_RECIPE;
            }
        }
        return null;
    ...
    private int onTheFlyGeneration() {
        ...
        return setOutputsAndTime(recipe.inputs, recipe.stackSize) ? FOUND_AND_SUCCESSFULLY_USED_RECIPE : FOUND_RECIPE_BUT_DID_NOT_MEET_REQUIREMENTS;
    }
    ...
    private int checkRecipeMap() {
        ...
        if (gt_recipe.isRecipeInputEqual(false, null, this.getAllInputs())) {
            if (gt_recipe.mSpecialValue == -100) {
                ...
                return setOutputsAndTime(gt_recipe.mOutputs, gt_recipe.mInputs[0].stackSize) ? FOUND_AND_SUCCESSFULLY_USED_RECIPE
    ...
    private Collection<DissassembleReference> findRecipeFromMachine() {
        ...
        List<DissassembleReference> possibleRecipes = GT_Recipe.GT_Recipe_Map.sAssemblerRecipes.mRecipeList.stream().filter(
                x -> Arrays.stream(x.mOutputs).anyMatch(y -> {
