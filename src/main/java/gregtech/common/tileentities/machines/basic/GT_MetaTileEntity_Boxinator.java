...
public class GT_MetaTileEntity_Boxinator extends GT_MetaTileEntity_BasicMachine {
    ...
    public GT_MetaTileEntity_Boxinator(int aID, String aName, String aNameRegional, int aTier) {
        super(aID, aName, aNameRegional, aTier, 1, "Puts things into Boxes", 2, 1, "Packager.png", "",
              TextureFactory.of(TextureFactory.of(OVERLAY_SIDE_BOXINATOR_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_SIDE_BOXINATOR_ACTIVE_GLOW).glow()
                                                                                                .build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_SIDE_BOXINATOR), TextureFactory.builder().addIcon(OVERLAY_SIDE_BOXINATOR_GLOW).glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_FRONT_BOXINATOR_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_FRONT_BOXINATOR_ACTIVE).glow()
                                                                                                 .build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_FRONT_BOXINATOR), TextureFactory.builder().addIcon(OVERLAY_FRONT_BOXINATOR_GLOW).glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_TOP_BOXINATOR_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_TOP_BOXINATOR_ACTIVE_GLOW).glow()
                                                                                               .build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_TOP_BOXINATOR), TextureFactory.builder().addIcon(OVERLAY_TOP_BOXINATOR_GLOW).glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_BOTTOM_BOXINATOR_ACTIVE), TextureFactory.builder().addIcon(OVERLAY_BOTTOM_BOXINATOR_ACTIVE_GLOW)
                                                                                                  .glow().build()),
              TextureFactory.of(TextureFactory.of(OVERLAY_BOTTOM_BOXINATOR), TextureFactory.builder().addIcon(OVERLAY_BOTTOM_BOXINATOR_GLOW).glow().build())
             );
    }
    ...
    protected boolean allowPutStackValidated(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, byte aSide, ItemStack aStack) {
        ...
        if ((ItemList.Schematic_1by1.isStackEqual(tInput1)) || (ItemList.Schematic_2by2.isStackEqual(tInput1)) || (
            ...
            if (GT_Recipe.GT_Recipe_Map.sBoxinatorRecipes.findRecipe(
                    getBaseMetaTileEntity(), true, gregtech.api.enums.GT_Values.V[mTier], null, GT_Utility.copyAmount(64L, aStack), tInput1) != null) {
                return true;
            }
            ...
            return ItemList.Schematic_3by3.isStackEqual(getInputAt(1)) && (
                    GT_ModHandler.getRecipeOutput(
                            aStack, aStack, aStack, aStack, aStack, aStack, aStack, aStack, aStack) != null
            );
        } else {
    ...
    public int checkRecipe() {
        ...
        if ((GT_Utility.isStackValid(tSlot0)) && (GT_Utility.isStackValid(tSlot1)) && (GT_Utility.getContainerItem(tSlot0, true) == null)) {
            ...
            if ((ItemList.Schematic_3by3.isStackEqual(tSlot1)) && (getInputAt(0).stackSize >= 9)) {
                ...
                this.mOutputItems[0] = tIsCached ? aOutputCache.copy() : GT_ModHandler.getRecipeOutput(
    ...
    private boolean hasValidCache(ItemStack mItem, int mType, boolean mClearOnFailure) {
        if (aInputCache != null && aOutputCache != null && aTypeCache == mType && aInputCache.isItemEqual(mItem) && ItemStack.areItemStackTagsEqual(mItem,
                                                                                                                                                    aInputCache
                                                                                                                                                   )) {
            return true;
        }
