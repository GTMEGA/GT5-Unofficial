...
public class GT_MetaTileEntity_MagicalEnergyAbsorber extends GT_MetaTileEntity_BasicGenerator implements MagicalEnergyBBListener {
    ...
    static class MagicalEnergyBB {
        ...
        MagicalEnergyBB(GT_MetaTileEntity_MagicalEnergyAbsorber aAbsorber, int aDefaultTier, int aMaxTier) {
            ...
            if (THAUMCRAFT_LOADED) {
                mAvailableAspects = new ArrayList<>(Aspect.aspects.size());
            }
        }
        ...
        void update() {
            ...
            if (mAbsorber.getBaseMetaTileEntity() == null) {
                return;
            }
            if (mAbsorber.getBaseMetaTileEntity().isInvalidTileEntity()) {
                return;
            }
            if (mAbsorber.getBaseMetaTileEntity().getWorld() == null) {
                return;
            }
            scanLivingCrystals();
        ...
        private AxisAlignedBB getAxisAlignedBB() {
            ...
            return AxisAlignedBB.getBoundingBox(tX - tRange, tY - tRange, tZ - tRange, tX + tRange, tY + tRange, tZ + tRange);
        }
                ...
                for (int rZ = -tRange; rZ <= tRange; rZ++) {
                    ...
                    for (int rY = rYMin; rY < rYMax; rY++) {
                        ...
                        if (tTile instanceof IAspectContainer) {
                            ...
                            if (tAspectList == null || tAspectList.aspects.isEmpty()) {
                                continue;
                            }
    ...
    public void onConfigLoad(GT_Config aConfig) {
        ...
        mMaxVisPerDrain = (int) Math.round(Math.sqrt(V[mTier] * 10000 / ((long) sEnergyFromVis * (getEfficiency() != 0 ? getEfficiency() : 100))));
    ...
    private static void sharedConfigLoad(GT_Config aConfig) {
        ...
        if (THAUMCRAFT_LOADED) {
            ...
            for (Aspect tAspect : Aspect.aspects.values()) {
                sAspectsEnergy.put(tAspect, Enums.getIfPresent(TC_Aspects.class, tAspect.getTag().toUpperCase(Locale.ENGLISH)).or(TC_Aspects.AER).mValue *
                                            sEnergyPerEssentia);
            }
        }
    }
    ...
    public void onScrewdriverRightClick(byte aSide, EntityPlayer aPlayer, float aX, float aY, float aZ) {
        ...
        GT_Utility.sendChatToPlayer(
                aPlayer, String.format(
                        GT_LanguageManager.addStringLocalization("Interaction_DESCRIPTION_MagicalEnergyAbsorber_Screwdriver", "Absorption range: %s blocks"),
                        mMagicalEnergyBB.getRange(), true
                                      ));
    ...
    public String[] getDescription() {
        ...
        description.add(LI + (sAllowMultipleEggs ? "A " : "An " + YELLOW + UNDERLINE + "EXCLUSIVE" + RESET) + GRAY + " " + LIGHT_PURPLE + "Dragon Egg" + GRAY +
                        " atop");
        ...
        if (THAUMCRAFT_LOADED) {
            ...
            description.add(
                    LI + (sEnergyPerEssentia * getEfficiency()) / 100 + EU_PER + LIGHT_PURPLE + "Essentia" + GRAY + " Aspect-Value from containers in range");
        }
        description.add(" ");
    ...
    public ITexture[] getFront(byte aColor) {
        return new ITexture[]{
                super.getFront(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC), TextureFactory.builder().addIcon(MACHINE_CASING_MAGIC_GLOW).glow().build(),
    ...
    public ITexture[] getBack(byte aColor) {
        return new ITexture[]{
                super.getBack(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC_FRONT), TextureFactory.builder().addIcon(MACHINE_CASING_MAGIC_FRONT_GLOW)
                                                                                                       .glow().build()
        };
    }
    ...
    public ITexture[] getBottom(byte aColor) {
        return new ITexture[]{
                super.getBottom(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC), TextureFactory.builder().addIcon(MACHINE_CASING_MAGIC_GLOW).glow().build()
        };
    }
    ...
    public ITexture[] getTop(byte aColor) {
        return new ITexture[]{
                super.getTop(aColor)[0], TextureFactory.of(MACHINE_CASING_DRAGONEGG)
        };
    }
    ...
    public ITexture[] getSides(byte aColor) {
        return new ITexture[]{
                super.getSides(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC), TextureFactory.builder().addIcon(MACHINE_CASING_MAGIC_GLOW).glow().build()
        };
    }
    ...
    public ITexture[] getFrontActive(byte aColor) {
        return new ITexture[]{
                super.getFrontActive(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC_ACTIVE), TextureFactory.builder().addIcon(
                MACHINE_CASING_MAGIC_ACTIVE_GLOW).glow().build(), OVERLAYS_ENERGY_OUT[mTier]
        };
    }
    ...
    public ITexture[] getBackActive(byte aColor) {
        return new ITexture[]{
                super.getBackActive(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC_FRONT_ACTIVE), TextureFactory.builder().addIcon(
                MACHINE_CASING_MAGIC_FRONT_ACTIVE_GLOW).glow().build()
        };
    }
    ...
    public ITexture[] getBottomActive(byte aColor) {
        return new ITexture[]{
                super.getBottomActive(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC_ACTIVE), TextureFactory.builder().addIcon(
                MACHINE_CASING_MAGIC_ACTIVE_GLOW).glow().build()
        };
    }
    ...
    public ITexture[] getTopActive(byte aColor) {
        return new ITexture[]{
                super.getTopActive(aColor)[0], TextureFactory.of(MACHINE_CASING_DRAGONEGG), TextureFactory.builder().addIcon(MACHINE_CASING_DRAGONEGG_GLOW)
                                                                                                          .glow().build()
        };
    }
    ...
    public ITexture[] getSidesActive(byte aColor) {
        return new ITexture[]{
                super.getSidesActive(aColor)[0], TextureFactory.of(MACHINE_CASING_MAGIC_ACTIVE), TextureFactory.builder().addIcon(
                MACHINE_CASING_MAGIC_ACTIVE_GLOW).glow().build()
        };
    }
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (aTick % mNextGenerateTickRate == 0) {
            ...
            aBaseMetaTileEntity.setActive(
                    aBaseMetaTileEntity.isAllowedToWork() && aBaseMetaTileEntity.getUniversalEnergyStored() >= maxEUOutput() + getMinimumStoredEU());
        }
    }
    ...
    private long generateEU() {
        ...
        if ((tEU = absorbFromEgg()) > 0) {
            return tEU;
        }
        if ((tEU = absorbFromEnderCrystals()) > 0) {
            return tEU;
        }
        if ((tEU = absorbFromEnchantedItems()) > 0) {
            return tEU;
        }
        if ((tEU = absorbFromVisNet()) > 0) {
            return tEU;
        }
        if ((tEU = absorbFromEssentiaContainers()) > 0) {
            return tEU;
        }
        return 0;
    }
    ...
    private long absorbFromEgg() {
        ...
        if (!sAllowMultipleEggs) {
            if (sActiveSiphon != null && sActiveSiphon != this && sActiveSiphon.getBaseMetaTileEntity() != null &&
                !sActiveSiphon.getBaseMetaTileEntity().isInvalidTileEntity() && sActiveSiphon.isChunkLoaded() && sActiveSiphon.hasEgg()) {
    ...
    private long absorbFromEnchantedItems() {
        ...
        if (!(isDisenchantableItem(tStack) || isEnchantedBook(tStack))) {
            return 0;
        }
        long tEU = 0;
        ...
        for (Map.Entry<?, ?> e : tMap.entrySet()) {
            if ((Integer) e.getKey() < Enchantment.enchantmentsList.length) {
                ...
                tEU += 1000000L * tLevel / ((long) tEnchantment.getMaxLevel() * tEnchantment.getWeight());
            }
        }
        ...
        if ((getBaseMetaTileEntity().getStoredEU() + tEU) < getBaseMetaTileEntity().getEUCapacity() && getBaseMetaTileEntity().addStackToSlot(
                getOutputSlot(), tStack)) {
    ...
    private long absorbFromEssentiaContainers() {
        ...
        for (int i = mAvailableEssentiaAspects.size() - 1; i >= 0 && tEUtoGen > 0; i--) {
            ...
            if (tAspectEU <= tEUtoGen && AspectSourceHelper.drainEssentia(
                    (TileEntity) getBaseMetaTileEntity(), aspect, ForgeDirection.UNKNOWN, mMagicalEnergyBB.getRange())) {
                tEUtoGen -= tAspectEU;
                tEU += tAspectEU;
            }
        }
