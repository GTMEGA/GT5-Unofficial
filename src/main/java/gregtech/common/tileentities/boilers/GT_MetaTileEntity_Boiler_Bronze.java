...
public class GT_MetaTileEntity_Boiler_Bronze extends GT_MetaTileEntity_Boiler {
    ...
    public GT_MetaTileEntity_Boiler_Bronze(int aID, String aName, String aNameRegional) {
        super(aID, aName, aNameRegional, new String[]{
                "An early way to get Steam Power", "Produces 120L of Steam per second", "Causes 20 Pollution per second"
        });
    }
    ...
    public ITexture[][][] getTextureSet(ITexture[] aTextures) {
        ...
        final ITexture[] texBottom = {TextureFactory.of(MACHINE_BRONZEBRICKS_BOTTOM)}, texTop = {
                TextureFactory.of(MACHINE_BRONZEBRICKS_TOP), TextureFactory.of(OVERLAY_PIPE)
        }, texSide = {TextureFactory.of(MACHINE_BRONZEBRICKS_SIDE), TextureFactory.of(OVERLAY_PIPE)}, texFront = {
                TextureFactory.of(MACHINE_BRONZEBRICKS_SIDE), TextureFactory.of(BOILER_FRONT), TextureFactory.builder().addIcon(BOILER_FRONT_GLOW)
                                                                                                             .glow().build()
        }, texFrontActive = {
                TextureFactory.of(MACHINE_BRONZEBRICKS_SIDE), TextureFactory.of(BOILER_FRONT_ACTIVE), TextureFactory.builder().addIcon(BOILER_FRONT_ACTIVE_GLOW)
                                                                                                                    .glow().build()
        };
        for (int i = 0; i < 17; i++) {
    ...
    protected void updateFuel(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if ((GT_Utility.isPartOfMaterials(this.mInventory[2], Materials.Coal) && !GT_Utility.isPartOfOrePrefix(this.mInventory[2], OrePrefixes.block)) ||
            ...
            GT_OreDictUnificator.isItemStackInstanceOf(this.mInventory[2], "fuelCoke") || GT_OreDictUnificator.isItemStackInstanceOf(
                this.mInventory[2], "fuelCactusCharcoal") || GT_OreDictUnificator.isItemStackInstanceOf(this.mInventory[2], "fuelCactusCoke") ||
            GT_OreDictUnificator.isItemStackInstanceOf(
                    this.mInventory[2], "fuelSugarCharcoal") || GT_OreDictUnificator.isItemStackInstanceOf(this.mInventory[2], "fuelSugarCoke")) {
            if ((TileEntityFurnace.getItemBurnTime(this.mInventory[2]) / 10) > 0) {
                ...
                if (XSTR.XSTR_INSTANCE.nextInt(
                        GT_Utility.isPartOfMaterials(this.mInventory[2], Materials.Coal) || GT_Utility.isPartOfMaterials(this.mInventory[2], Materials.Charcoal)
                        ? 3 : GT_Utility.isPartOfMaterials(this.mInventory[2], Materials.Lignite) ? 8 : 2) == 0) {
                    aBaseMetaTileEntity.addStackToSlot(
                            3, GT_OreDictUnificator.get(OrePrefixes.dustTiny, (
                                                                              ...
                                                                              ) ? Materials.DarkAsh : Materials.Ash, 1L));
                }
            }
        } else if (
            //If its a block of the following materials
                ...
                (
                        ...
                        (
                                Block.getBlockFromItem(this.mInventory[2].getItem()).getUnlocalizedName().toLowerCase().contains("tile") &&
            ...
            if ((TileEntityFurnace.getItemBurnTime(this.mInventory[2]) / 10) > 0) {
                ...
                aBaseMetaTileEntity.decrStackSize(2, 1);
                aBaseMetaTileEntity.addStackToSlot(
                        3, GT_OreDictUnificator.get(OrePrefixes.dust, (
                                                                              GT_Utility.isPartOfMaterials(this.mInventory[2], Materials.Lignite) ||
                                                                              GT_Utility.isPartOfMaterials(this.mInventory[2], Materials.Coal) ||
                                                                              Block.getBlockFromItem(this.mInventory[2].getItem()).getUnlocalizedName()
                                                                                   .toLowerCase().contains("coal") || Block.getBlockFromItem(
                                                                                                                                   this.mInventory[2].getItem()).getUnlocalizedName().toLowerCase()
                                                                                                                           .contains("lignite")
                                                                      ) ? Materials.DarkAsh : Materials.Ash, 1L));
            }
            //enables every other fuel with at least 2000 burntime as a fuel, i.e. peat, Magic/Solid Super Fuel, Coal Singularities, Nitor, while bucket of
            // creosite should be blocked same goes for lava
        } else if ((TileEntityFurnace.getItemBurnTime(this.mInventory[2])) >= 2000 && !(
                this.mInventory[2].getUnlocalizedName().toLowerCase().contains("bucket") || this.mInventory[2].getUnlocalizedName().toLowerCase().contains(
                        "cell")
        )) {
            this.mProcessingEnergy += (TileEntityFurnace.getItemBurnTime(this.mInventory[2]) / 10);
            aBaseMetaTileEntity.decrStackSize(2, 1);
            ...
            if (XSTR.XSTR_INSTANCE.nextInt(2) == 0) {
                aBaseMetaTileEntity.addStackToSlot(3, GT_OreDictUnificator.get((
