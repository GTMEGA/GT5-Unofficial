...
public class GT_Worldgen_GT_Ore_SmallPieces extends GT_Worldgen {
    ...
    public boolean executeWorldgen(
        ...
        if (this.mMeta > 0) {
            ...
            for (int i = 0; i < j; i++) {
                GT_Block_Ore_Abstract.setOreBlock(aWorld, aChunkX + 8 + aRandom.nextInt(16), this.mMinY + aRandom.nextInt(Math.max(1, this.mMaxY - this.mMinY)),
                                                  aChunkZ + 8 + aRandom.nextInt(16), GregTech_API.sGeneratedMaterials[this.mMeta], isUnderdark, OreSize.Small
                                                 );
                count++;
            }
        }
        if (debugSmallOres) {
            GT_Log.out.println(
                    "Small Ore:" + this.mWorldGenName + " @ dim=" + aDimensionType + " mX=" + aChunkX / 16 + " mZ=" + aChunkZ / 16 + " ore=" + count);
        }
        return true;
    }
