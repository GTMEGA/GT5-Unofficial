...
public class GT_Worldgen_GT_Ore_Layer extends GT_Worldgen {
    ...
    public GT_Worldgen_GT_Ore_Layer(
        ...
        short tPrimaryMeta = (short) GregTech_API.sWorldgenFile.get(aTextWorldgen + this.mWorldGenName, "OrePrimaryLayer", aPrimary.mMetaItemSubID);
        this.mPrimary = tPrimaryMeta > 0 ? GregTech_API.sGeneratedMaterials[tPrimaryMeta] : null;

        short tSecondaryMeta = (short) GregTech_API.sWorldgenFile.get(aTextWorldgen + this.mWorldGenName, "OreSecondaryLayer", aSecondary.mMetaItemSubID);
        this.mSecondary = tSecondaryMeta > 0 ? GregTech_API.sGeneratedMaterials[tSecondaryMeta] : null;

        short tBetweenMeta = (short) GregTech_API.sWorldgenFile.get(aTextWorldgen + this.mWorldGenName, "OreSporadiclyInbetween", aBetween.mMetaItemSubID);
        this.mBetween = tBetweenMeta > 0 ? GregTech_API.sGeneratedMaterials[tBetweenMeta] : null;

        short tSporadicMeta = (short) GregTech_API.sWorldgenFile.get(aTextWorldgen + this.mWorldGenName, "OreSporaticlyAround", aSporadic.mMetaItemSubID);
    ...
    public int executeWorldgenChunkified(
        ...
        if (!isGenerationAllowed(
                aWorld, aDimensionType, ((aDimensionType == -1) && (this.mNether)) || ((aDimensionType == 0) && (this.mOverworld)) ||
        ...
        /*if (!((aWorld.provider.getDimensionName().equalsIgnoreCase("Overworld")) || (aWorld.provider.getDimensionName().equalsIgnoreCase("Nether"))||
        (aWorld.provider.getDimensionName().equalsIgnoreCase("Underdark"))||(aWorld.provider.getDimensionName().equalsIgnoreCase("Twilight Forest"))||(aWorld
        .provider.getDimensionName().equalsIgnoreCase("Underdark"))||(aWorld.provider.getDimensionName().equalsIgnoreCase("The End"))))
        	return WRONG_DIMENSION;*/
        ...
        if (wX >= eX) {  //No overlap between orevein and this chunk exists in X
            if (tBlock.isReplaceableOreGen(aWorld, aChunkX + 7, tMinY, aChunkZ + 9, Blocks.stone) || tBlock.isReplaceableOreGen(
                    aWorld, aChunkX + 7, tMinY, aChunkZ + 9, Blocks.netherrack) || tBlock.isReplaceableOreGen(
                    aWorld, aChunkX + 7, tMinY, aChunkZ + 9, Blocks.end_stone) || tBlock.isReplaceableOreGen(
                    aWorld, aChunkX + 7, tMinY, aChunkZ + 9, GregTech_API.sBlockGranites) || tBlock.isReplaceableOreGen(
                    aWorld, aChunkX + 7, tMinY, aChunkZ + 9, GregTech_API.sBlockStones)) {
        ...
        if (debugOrevein) {
            GT_Log.out.print(
                    "Trying Orevein:" + this.mWorldGenName + " Dimension=" + tDimensionName + " mX=" + aChunkX / 16 + " mZ=" + aChunkZ / 16 + " oreseedX=" +
                    aSeedX / 16 + " oreseedZ=" + aSeedZ / 16 + " cY=" + tMinY);
        }
        // Adjust the density down the more chunks we are away from the oreseed.  The 5 chunks surrounding the seed should always be max density due to
        // truncation of Math.sqrt().
        int localDensity = Math.max(
                1, this.mDensity / ((int) Math.sqrt(2 + Math.pow(aChunkX / 16 - aSeedX / 16, 2) + Math.pow(aChunkZ / 16 - aSeedZ / 16, 2))));
        ...
        for (int tX = wX; tX < eX; tX++) {
            ...
            for (int tZ = nZ; tZ < sZ; tZ++) {
                ...
                } else if ((aRandom.nextInt(7) == 0) && ((aRandom.nextInt(placeZ) == 0) || (aRandom.nextInt(placeX) == 0)) &&
        ...
        for (int tX = wX; tX < eX; tX++) {
            ...
            for (int tZ = nZ; tZ < sZ; tZ++) {
                ...
                if ((aRandom.nextInt(2) == 0) && ((aRandom.nextInt(placeZ) == 0) || (aRandom.nextInt(placeX) == 0)) &&
                    (this.mBetween != null)) {  // Between are reduce by 1/2 to compensate
        ...
        if (debugOrevein) {
            GT_Log.out.println(
                    " wXVein" + wXVein + " eXVein" + eXVein + " nZVein" + nZVein + " sZVein" + sZVein + " locDen=" + localDensity + " Den=" + this.mDensity +
                    " Sec=" + placeCount[1] + " Spo=" + placeCount[3] + " Bet=" + placeCount[2] + " Pri=" + placeCount[0]);
        }
