...
public class GT_Cover_FluidRegulator extends GT_CoverBehavior {
    ...
    private class GUI extends GT_GUICover {
        ...
        public void drawExtras(int mouseX, int mouseY, float parTicks) {
            ...
            this.getFontRenderer().drawString(trans("200", " L"), startX + spaceX * 4, 4 + startY + spaceY, 0xFF555555);
            ...
            if (warn) {
                this.getFontRenderer().drawString(
                        String.format(trans("210", "Average: %.2f L/sec"), speed * 20d / tickRate), startX, 4 + startY + spaceY * 3, 0xffff0000);
            } else {
                this.getFontRenderer().drawString(
                        String.format(trans("210", "Average: %.2f L/sec"), speed * 20d / tickRate), startX, 4 + startY + spaceY * 3, 0xFF555555);
            }
        }
    ...
    public int onCoverScrewdriverclick(
            byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity, EntityPlayer aPlayer, float aX, float aY, float aZ
                                      ) {
        if (GT_Utility.getClickedFacingCoords(aSide, aX, aY, aZ)[0] >= 0.5F) {
    ...
    private int adjustSpeed(EntityPlayer aPlayer, int aCoverVariable, int scale) {
        ...
        if (tTickRate == 1) {
            GT_Utility.sendChatToPlayer(aPlayer, trans("048", "Pump speed: ") + tSpeed + trans("049", "L/tick ") + tSpeed * 20 + trans("050", "L/sec"));
        } else {
            GT_Utility.sendChatToPlayer(aPlayer, String.format(trans("207", "Pump speed: %dL every %d ticks, %.2f L/sec on average"), tSpeed, tTickRate,
