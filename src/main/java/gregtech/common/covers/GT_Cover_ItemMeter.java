...
public class GT_Cover_ItemMeter extends GT_CoverBehavior {
    ...
    public int doCoverThings(byte aSide, byte aInputRedstone, int aCoverID, int aCoverVariable, ICoverable aTileEntity, long aTimer) {
        ...
        if (mte instanceof GT_MetaTileEntity_DigitalChestBase) {
            ...
            int[] tSlots = (aCoverVariable & SLOT_MASK) > 0 ? new int[]{(aCoverVariable & SLOT_MASK) - 1} : aTileEntity.getAccessibleSlotsFromSide(aSide);
            ...
            for (int i : tSlots) {
                if (i >= 0 && i < aTileEntity.getSizeInventory()) {
                    ...
                    if (tStack != null) {
                        tUsed += ((long) tStack.stackSize << 6) / tStack.getMaxStackSize();
                    }
                }
            }
        }
