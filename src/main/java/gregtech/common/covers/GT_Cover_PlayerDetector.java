...
public class GT_Cover_PlayerDetector extends GT_CoverBehavior {
    ...
    private class GUI extends GT_GUICover {
        ...
        public GUI(byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity) {
            ...
            new GT_GuiIconCheckButton(this, 0, startX, startY, GT_GuiIcon.CHECKMARK, null).setTooltipText(
                    trans("068", "Emit if any Player is close"));
            new GT_GuiIconCheckButton(this, 1, startX, startY + spaceY, GT_GuiIcon.CHECKMARK, null).setTooltipText(
                    trans("069", "Emit if other Player is close"));
            new GT_GuiIconCheckButton(this, 2, startX, startY + spaceY * 2, GT_GuiIcon.CHECKMARK, null).setTooltipText(
                    trans("070", "Emit if you are close"));
        }

        @Override
        public void drawExtras(int mouseX, int mouseY, float parTicks) {
            super.drawExtras(mouseX, mouseY, parTicks);
            this.fontRendererObj.drawString("Any player", startX + spaceX, 4 + startY, 0xFF555555);
            this.fontRendererObj.drawString("Other players", startX + spaceX, 4 + startY + spaceY, 0xFF555555);
            this.fontRendererObj.drawString("Only owner", startX + spaceX, 4 + startY + spaceY * 2, 0xFF555555);
        }
    ...
    public int doCoverThings(byte aSide, byte aInputRedstone, int aCoverID, int aCoverVariable, ICoverable aTileEntity, long aTimer) {
        ...
        for (Object tObject : aTileEntity.getWorld().playerEntities) {
            if ((tObject instanceof EntityPlayerMP)) {
                ...
                int dist = Math.max(
                        1, (int) tEntity.getDistance(aTileEntity.getXCoord() + 0.5D, aTileEntity.getYCoord() + 0.5D, aTileEntity.getZCoord() + 0.5D));
                if (dist < range) {
                    if (aCoverVariable == 0) {
                        playerDetected = true;
                        break;
