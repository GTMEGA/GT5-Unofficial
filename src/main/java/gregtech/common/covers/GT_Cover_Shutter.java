...
public class GT_Cover_Shutter extends GT_CoverBehavior {
    ...
    private class GUI extends GT_GUICover {
        ...
        public GUI(byte aSide, int aCoverID, int aCoverVariable, ICoverable aTileEntity) {
            ...
            new GT_GuiIconCheckButton(this, 3, startX, startY + spaceY * 3, GT_GuiIcon.CHECKMARK, null);
        }

        @Override
        public void drawExtras(int mouseX, int mouseY, float parTicks) {
            super.drawExtras(mouseX, mouseY, parTicks);
            this.fontRendererObj.drawString(trans("082", "Open if work enabled"), 3 + startX + spaceX, 4 + startY, 0xFF555555);
            this.fontRendererObj.drawString(trans("083", "Open if work disabled"), 3 + startX + spaceX, 4 + startY + spaceY, 0xFF555555);
            this.fontRendererObj.drawString(trans("084", "Only Output allowed"), 3 + startX + spaceX, 4 + startY + spaceY * 2, 0xFF555555);
            this.fontRendererObj.drawString(trans("085", "Only Input allowed"), 3 + startX + spaceX, 4 + startY + spaceY * 3, 0xFF555555);
        }
    ...
    public boolean letsItemsOut(byte aSide, int aCoverID, int aCoverVariable, int aSlot, ICoverable aTileEntity) {
        return aCoverVariable >= 2 ? aCoverVariable == 2 : !(aTileEntity instanceof IMachineProgress) ||
                                                           ((IMachineProgress) aTileEntity).isAllowedToWork() == (aCoverVariable % 2 == 0);
    }

    @Override
    public boolean letsItemsIn(byte aSide, int aCoverID, int aCoverVariable, int aSlot, ICoverable aTileEntity) {
        return aCoverVariable >= 2 ? aCoverVariable == 3 : !(aTileEntity instanceof IMachineProgress) ||
                                                           ((IMachineProgress) aTileEntity).isAllowedToWork() == (aCoverVariable % 2 == 0);
    }
