...
public class GT_Pollution {
            ...
            if (tPollution > 400000) {//Spread Pollution
                ...
                if (tPollution > GT_Mod.gregtechproxy.mPollutionSmogLimit) {
                    AxisAlignedBB chunk = AxisAlignedBB.getBoundingBox(
                            actualPos.chunkXPos << 4, 0, actualPos.chunkZPos << 4, (actualPos.chunkXPos << 4) + 16, 256, (actualPos.chunkZPos << 4) + 16);
                    ...
                    if (tPollution > GT_Mod.gregtechproxy.mPollutionPoisonLimit) {
                        //AxisAlignedBB chunk = AxisAlignedBB.getBoundingBox(tPos.chunkPosX*16, 0, tPos.chunkPosZ*16, tPos.chunkPosX*16+16, 256, tPos
                        // .chunkPosZ*16+16);
    ...
    private static void damageBlock(World world, int x, int y, int z, boolean sourRain) {
        ...
        if (tBlock == Blocks.waterlily || tBlock == Blocks.wheat || tBlock == Blocks.cactus || tBlock.getMaterial() == Material.cactus ||
            tBlock == Blocks.melon_block || tBlock == Blocks.melon_stem) {
            tBlock.dropBlockAsItem(world, x, y, z, tMeta, 0);
            world.setBlockToAir(x, y, z);
        }
        if (tBlock == Blocks.red_flower || tBlock == Blocks.yellow_flower || tBlock == Blocks.carrots || tBlock == Blocks.potatoes ||
            tBlock == Blocks.pumpkin || tBlock == Blocks.pumpkin_stem) {
            tBlock.dropBlockAsItem(world, x, y, z, tMeta, 0);
            world.setBlockToAir(x, y, z);
        }
        ...
        if (sourRain && world.isRaining() && (tBlock == Blocks.stone || tBlock == Blocks.gravel || tBlock == Blocks.cobblestone) && world.getBlock(
                x, y + 1, z) == Blocks.air && world.canBlockSeeTheSky(x, y, z)) {
    ...
    public static void addPollution(IGregTechTileEntity te, int aPollution) {
        if (!GT_Mod.gregtechproxy.mPollution || aPollution == 0 || te.isClientSide()) {
            return;
        }
    ...
    public static void addPollution(World w, int chunkX, int chunkZ, int aPollution) {
        if (!GT_Mod.gregtechproxy.mPollution || aPollution == 0 || w.isRemote) {
            return;
        }
    ...
    @Deprecated /*Don't use it... too weird way of passing position*/ public static void addPollution(World aWorld, ChunkPosition aPos, int aPollution) {
    ...
    public static void addPollution(Chunk ch, int aPollution) {
        if (!GT_Mod.gregtechproxy.mPollution || aPollution == 0 || ch.worldObj.isRemote) {
            return;
        }
