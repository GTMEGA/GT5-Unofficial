...
public class GT_UndergroundOil {
    ...
    private static class GT_UndergroundOilStore extends GT_ChunkAssociatedData<ChunkData> {
        ...
        @Override
        protected GT_UndergroundOil.ChunkData readElement(DataInput input, int version, World world, int chunkX, int chunkZ)
                throws IOException {
            /* see class javadoc for explanation */
            if (version != 0) {
            ...
            return new GT_UndergroundOil.ChunkData(
                    amount, GT_Mod.gregtechproxy.mUndergroundOil.GetDimension(world.provider.dimensionId).getUOFluid(veinKey), veinKey);
        }
