...
public class GT_PollutionRenderer {
    ...
    public void renderGTPollutionFog(EntityViewRenderEvent.FogDensity event) {
        ...
        if (event.entity.isPotionActive(Potion.blindness) || (fogIntensityLastTick < FOG_START_EXP_RATIO) || event.block.getMaterial() == Material.water ||
    ...
    public void onRenderTick(TickEvent.RenderTickEvent event) {
        ...
        if (event.phase == TickEvent.Phase.START) {
            ...
            drawPollution(
                    "Pollution: " + pollutionMap.getPollution(Minecraft.getMinecraft().thePlayer.lastTickPosX, Minecraft.getMinecraft().thePlayer.lastTickPosZ),
                    20
                         );
